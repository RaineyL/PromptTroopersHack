...
function App() {
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-roadmap"><div className="nav-label">BUILD PROGRESS</div>{stages.map((item, index) => <div key={item.name}><span className={index === 0 ? 'roadmap-dot ready' : 'roadmap-dot'}/><span>{item.name}</span><small>{index === 0 ? 'Ready' : 'Planned'}</small></div>)}</div>
      ...
      <main id="main-content">
        ...
        <div hidden={view !== 'pipeline'}>
          <section className="pipeline-map" aria-label="Verification pipeline">
            <div className="section-heading"><h2>The verification pipeline</h2><span className="subtle">1 of 4 stages implemented</span></div>
            <ol>{stages.map((item, index) => <li key={item.name}><div className={`stage-icon ${index === 0 ? 'active' : ''}`}><Icon name={item.icon}/></div><div><div className="stage-label"><span>0{index + 1}</span><strong>{item.name}</strong></div><p>{item.description}</p><span className={`badge ${index === 0 ? 'green' : 'neutral'}`}>{index === 0 ? 'Ready to run' : 'Not implemented'}</span></div>{index < 3 && <span className="stage-arrow"><Icon name="arrow"/></span>}</li>)}</ol>
