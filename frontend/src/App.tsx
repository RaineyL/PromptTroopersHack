...
const implemented = new Set([0, 1, 2])
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-roadmap"><div className="nav-label">BUILD PROGRESS</div>{stages.map((item, index) => <div key={item.name}><span className={implemented.has(index) ? 'roadmap-dot ready' : 'roadmap-dot'}/><span>{item.name}</span><small>{implemented.has(index) ? 'Ready' : 'Planned'}</small></div>)}</div>
      ...
      <main id="main-content">
        ...
        <div hidden={view !== 'pipeline'}>
          <section className="pipeline-map" aria-label="Verification pipeline">
            <div className="section-heading"><h2>The verification pipeline</h2><span className="subtle">{implemented.size} of 4 stages implemented</span></div>
            <ol>{stages.map((item, index) => <li key={item.name}><div className={`stage-icon ${implemented.has(index) ? 'active' : ''}`}><Icon name={item.icon}/></div><div><div className="stage-label"><span>0{index + 1}</span><strong>{item.name}</strong></div><p>{item.description}</p><span className={`badge ${implemented.has(index) ? 'green' : 'neutral'}`}>{implemented.has(index) ? 'Ready to run' : 'Not implemented'}</span></div>{index < 3 && <span className="stage-arrow"><Icon name="arrow"/></span>}</li>)}</ol>
            <div className="pipeline-note"><Icon name="info"/><span>Only document-comparison requests continue beyond classification. Comparison runs on extracted documents; anything it cannot decide goes to human review.</span><a href="#debug">Test a stage <Icon name="arrow"/></a></div>
          </section>
          <ClassificationWorkspace onExtractions={setExtractions}/>
          <ComparisonWorkspace key={JSON.stringify(extractions)} extractions={extractions}/>
        </div>
        <div hidden={view !== 'debug'}>
          ...
          <div className="debug-layout">
            ...
            <div className="debug-content">
              ...
              <div hidden={stage !== 2}><ComparisonWorkspace debug/></div>
