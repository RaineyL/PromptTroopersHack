...
export function PipelineDashboard({ onSaveExtraction }: { onSaveExtraction: (result: ExtractionResult, source: 'Pipeline') => void }) {
  ...
  const [reviewStatus, setReviewStatus] = useState<'' | 'OK' | 'MISMATCH' | 'UNABLE_TO_VERIFY'>('')
  ...
  return <div className="pipeline-dashboard">
    ...
    <section className="panel pipeline-results"><div className="section-heading"><div><p className="eyebrow">02 · Results</p><h2>Email queue</h2></div><span className="subtle">Classify → Extract → Compare → Report</span></div>
      ...
      {pageCount > 1 && <div className="pagination"><button className="secondary" disabled={currentPage === 0} onClick={() => { setPage(currentPage - 1); window.scrollTo({ top: 0, behavior: 'smooth' }) }}>Previous</button><span>Page {currentPage + 1} of {pageCount} · {visible.length} emails</span><button className="secondary" disabled={currentPage + 1 === pageCount} onClick={() => { setPage(currentPage + 1); window.scrollTo({ top: 0, behavior: 'smooth' }) }}>Next</button></div>}
