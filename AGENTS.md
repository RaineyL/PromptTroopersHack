...
- Pipeline intake uploads a bounded ZIP of inbox emails and attachments into a temporary backend session; Debug uses the Docker inbox HTTP service and also supports optional attachment retrieval and manual JSON input.
- Keep **Pipeline** as the default workspace. Show the full flow: Classify → Extract → Compare → Report. **Debug mode** is a separate workspace for testing each stage independently; its inputs, results, and review decisions must not change the main pipeline session. Completed Pipeline and Extraction Debug snapshots may be saved in browser local storage and loaded as copies in Comparison Debug; preserve extraction evidence and review flags.
- Mark stage availability explicitly. Classification, extraction, comparison, and JSON reporting are implemented in Pipeline and Debug. Comparison consumes extraction responses directly and preserves unresolved extraction findings for human review. Report combines current classification and comparison results into the sample-submission shape in the browser; unresolved model/review decisions export as `NEEDS_REVIEW`, while missing classifications or unrun comparisons still block export. Extraction Debug uses ground-truth category solely to filter eligible IDs and requests document contents from Docker; no defect labels enter extraction. Never present fabricated processed counts, completed comparisons, or mismatch-free reports.
