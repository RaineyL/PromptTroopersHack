...
- Mark stage availability explicitly. Classification and comparison are implemented; extraction and the discrepancy report are not. Planned stage controls remain disabled and describe their expected inputs and outputs. Never present fabricated processed counts, completed comparisons, or mismatch-free reports. An email whose documents could not be read, or were not the expected documents, is shown as needing review — never as a clean result.
